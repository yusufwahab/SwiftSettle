  ...
  return (
    <section className="px-6 py-20 sm:px-10 lg:px-16">
      <div className="mx-auto max-w-6xl">
        <h2 className="text-center text-3xl font-bold text-ink">How SwiftSettle Works</h2>
        ...
        <div className="mt-16 space-y-16">
          {steps.map((step, index) => {
            ...
            return (
              ...
              >
                <div>
                  <span className="text-5xl font-bold text-border-strong">{step.number}</span>
                  <h3 className="mt-4 text-xl font-bold text-ink">{step.heading}</h3>
                  <p className="mt-3 text-base text-muted">{step.description}</p>
                </div>
                <Photo slot={step.slot} className="h-[280px] border border-border" />
              </div>
            );
          })}
        </div>
