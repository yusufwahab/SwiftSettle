...
- **Payout requests** — "Simulate Delivery" reports a fake completed order; "Request Payout" bundles every pending order into one request; an **Admin: Payouts** page (only visible to accounts with admin access) processes it with an amount that can deliberately differ from what was requested, demonstrating matched/underpaid/overpaid/unmatched reconciliation. Any account can self-grant admin access from Settings — no separate admin login exists.
